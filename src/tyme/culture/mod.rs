...
impl God {
  ...
  pub fn get_day_gods(month: SixtyCycle, day: SixtyCycle) -> Vec<Self> {
    let month_idx = month.get_earth_branch().next(-2).get_index();
    let day_idx = day.get_index();

    DAY_GODS_LOOKUP[month_idx][day_idx]
      .iter()
...
impl Phase {
  ...
  fn get_start_solar_time(&self) -> SolarTime {
    let n: isize = floor((self.lunar_year - 2000) as f64 * 365.2422 / 29.53058886) as isize;
    let mut i: isize = 0;
