...
impl RabByungYear {
  ...
  pub fn from_element_zodiac(
    ...
    Err(format!(
      "illegal rab-byung element {}, zodiac {}",
      element, zodiac
    ))
  }
  ...
  pub fn get_element(&self) -> RabByungElement {
    RabByungElement::from_index(
      self.sixty_cycle.get_heaven_stem().get_element().get_index() as isize
    )
  }
...
/// 查找藏历月数据
fn find_month_days(key: usize) -> Vec<isize> {
  for (k, data) in RAB_BYUNG_MONTH_DAYS.iter() {
    if *k == key {
      return data.to_vec();
    }
  }
...
impl RabByungMonth {
  ...
  pub fn from_element_zodiac(
    ...
    Self::new(
      RabByungYear::from_element_zodiac(rab_byung_index, element, zodiac)?,
      month,
    )
  }
...
impl RabByungDay {
  ...
  pub fn from_element_zodiac(
    ...
    Self::new(
      RabByungMonth::from_element_zodiac(rab_byung_index, element, zodiac, month)?,
      day,
    )
  }
...
mod tests {
  ...
  fn test6() {
    ...
    assert_eq!(
      "1951年1月8日",
      RabByungDay::from_element_zodiac(
        15,
